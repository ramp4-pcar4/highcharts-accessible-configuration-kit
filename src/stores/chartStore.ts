...
export const useChartStore = defineStore('chartProperties', {
    ...
    actions: {
        ...
        setupConfig(
            seriesNames: { en: string; fr: string }[],
            cats: string[],
            seriesData: number[][],
            categoryLabel = { en: '', fr: '' }
        ): void {
        ...
        updateConfig(
            type: string,
            series: { en: string; fr: string }[],
            headers: { en: string; fr: string }[],
            gridData: string[][],
        ...
        updateHeader(colIdx: number, name: { en: string; fr: string }): void {
            if (colIdx === 0) {
        ...
        updateLineChart(seriesNames: { en: string; fr: string }[], seriesData: number[][]): void {
        ...
        updateBarChart(seriesNames: { en: string; fr: string }[], seriesData: number[][]): void {
            this.chartConfig.series = this.chartConfig.series.map((series, index) =>
                seriesNames.some((name) => name[this.activeLang] === series.name[this.activeLang])
                    ? {
                          name: series.name,
        ...
        updateColumnChart(seriesNames: { en: string; fr: string }[], seriesData: number[][]): void {
            this.chartConfig.series = this.chartConfig.series.map((series, index) =>
                seriesNames.some((name) => name[this.activeLang] === series.name[this.activeLang])
                    ? {
                          name: series.name,
        ...
        updateAreaChart(seriesNames: { en: string; fr: string }[], seriesData: number[][]): void {
            this.chartConfig.series = this.chartConfig.series.map((series, index) =>
                seriesNames.some((name) => name[this.activeLang] === series.name[this.activeLang])
                    ? {
                          name: series.name,
        ...
        updateSplineChart(seriesNames: { en: string; fr: string }[], seriesData: number[][]): void {
            this.chartConfig.series = this.chartConfig.series.map((series, index) =>
                seriesNames.some((name) => name[this.activeLang] === series.name[this.activeLang])
                    ? {
                          name: series.name,
        ...
        toBilingual(value: string | { en?: string; fr?: string }): { en: string; fr: string } {
