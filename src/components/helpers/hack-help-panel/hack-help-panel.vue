<template>
    ...
    >
        <header class="flex justify-between border-b pb-4 top-0 items-center">
            <h2 class="text-lg font-semibold pl-4 pt-2">
                <span class="help-title">{{ $t('HACK.help.title') }}</span>
                <span class="help-title-mobile">{{ $t('HACK.help.title.mobile') }}</span>
            </h2>
        ...
        <div class="help-section-headers pl-3 pr-3 overflow-y-auto flex-grow max-w-none">
            <div v-for="(section, i) in helpSections" :key="i">
                <div
                    ...
                    @keydown.enter.space.prevent="section.expanded = !section.expanded"
                >
                    <!-- name -->
...
import { helpEnMd, helpFrMd, helpImages } from '@/hack-help/assets';
...
export default defineComponent({
    ...
    methods: {
      ...
      <img src="${imageSrc}" alt="${text}" style="display:block; max-width:none; width:auto; max-height:400px;" />
            ...
            this.helpMd = this.$i18n.locale === 'fr' ? helpFrMd : helpEnMd;

            this.helpMd = this.helpMd.replace(new RegExp(String.fromCharCode(13), 'g'), '');
            let section;
...
.help-title-mobile { display: none;}

@media (max-width: 640px) {
    .help-title { display: none;}
    .help-title-mobile { display: inline;}
}
</style>
